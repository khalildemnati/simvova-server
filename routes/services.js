...
router.get("/", async (req, res) => {
  try {
    // استخدم رمز دولة صالح مثل "usa" لتفادي الخطأ
    ...
    Object.keys(data).forEach((service) => {
      Object.keys(data[service]).forEach((operator) => {
        const originalPrice = parseFloat(data[service][operator].cost || 0);
        const newPrice = (originalPrice * PROFIT_MULTIPLIER).toFixed(2);
        data[service][operator].cost = newPrice;
        data[service][operator].currency = "USD";
      });
    });
